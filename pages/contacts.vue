<template>
  <div class="max-w-[1700px] m-auto px-16">
    <h1>The mask-image Property</h1>
    <svg
      width="1800px"
      height="1600px"
      version="1.1"
      xmlns="http://www.w3.org/2000/svg"
      xmlns:xlink="http://www.w3.org/1999/xlink"
      xml:space="preserve"
      xmlns:serif="http://www.serif.com/"
      style="
        fill-rule: evenodd;
        clip-rule: evenodd;
        stroke-linejoin: round;
        stroke-miterlimit: 2;
      "
    >
      <defs>
        <mask id="spiralMask">
          <rect width="100%" height="100%" fill="black" />
          <!-- <path d="m351 487c0 8-11 4-14-1-6-11 4-24 15-27 19-5 37 11 40 30 4 27-18 50-44 53-35 4-64-25-66-59-3-42 32-77 73-79 50-3 90 39 92 88 2 57-46 104-102 105-65 2-117-53-119-117-1-72 60-131 131-132 80-1 144 67 145 146 1 87-74 158-160 158-95 0-171-81-171-175 0-102 88-185 190-184 110 1 198 95 197 204C557 615 456 709 340 708 215 706 115 598 117 475 119 342 233 236 364 238 504 240 616 361 614 500 611 648 484 766 337 763 182 760 58 626 61 472 65 309 206 179 367 183c170 4 306 151 302 320-4 178-158 319-335 315"/> -->
          <path
          d="m351 487c0 8-11 4-14-1-6-11 4-24 15-27 19-5 37 11 40 30 4 27-18 50-44 53-35 4-64-25-66-59-3-42 32-77 73-79 50-3 90 39 92 88 2 57-46 104-102 105-65 2-117-53-119-117-1-72 60-131 131-132 80-1 144 67 145 146 1 87-74 158-160 158-95 0-171-81-171-175 0-102 88-185 190-184 110 1 198 95 197 204C557 615 456 709 340 708 215 706 115 598 117 475 119 342 233 236 364 238 504 240 616 361 614 500 611 648 484 766 337 763 182 760 58 626 61 472 65 309 206 179 367 183c170 4 306 151 302 320-4 178-158 319-335 315"
            stroke="white"
            fill="none"
          />
        </mask>
      </defs>
      <image
        xlink:href="https://www.w3schools.com/css/img_5terre.jpg"
        x="0"
        y="0"
        height="600px"
        width="800px"
        mask="url(#spiralMask)"
      />
    </svg>

    <ContentDoc />
  </div>
</template>

<script lang="ts" setup>
import { onMounted } from "vue";
import { gsap } from "gsap";
import { ScrollTrigger } from "gsap/ScrollTrigger";
gsap.registerPlugin(ScrollTrigger);
import { MotionPathPlugin } from "gsap/MotionPathPlugin";
gsap.registerPlugin(MotionPathPlugin);

onMounted(() => {
  gsap.to("#spiralMask path", {
    transformOrigin: "50% 50%",
    duration: 5,
    strokeWidth: 200,
    rotation: 360,
    repeat: -1,
    yoyo: true,
  });
});
</script>

<style scoped>
.spirale-clip {
  width: 600px;
  height: 600px;
  /* background-color: red; */
  background: url("https://www.w3schools.com/css/img_5terre.jpg") no-repeat center/cover;
  mask: url(#spiraleMask);
  -webkit-mask: url(#spiraleMask);
  /* animation: animateClipPath 5s infinite; */
}

/* #spiralMask path {
  animation: animateClipPath 5s infinite;
} */

@keyframes animateMask {
  0% {
    transform: scale(1);
  }
  50% {
    transform: scale(1.2);
  }
  100% {
    transform: scale(1);
  }
}

@keyframes animateClipPath {
  0% {
    stroke-width: 1;
  }
  50% {
    stroke-width: 30;
  }
  100% {
    stroke-width: 1;
  }
}
</style>
